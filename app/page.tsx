...
const ALL_INFO: Record<string, { muscles: string; steps: string[]; tip: string; img?: string }> = {
  ...
  "Remo / Bici 85%": { muscles: "Cardiovascular · Tren inferior · Espalda", steps: ["Remo o bicicleta estática.", "Alcanzá el 85% de tu FC máxima (220 - tu edad).", "Mantené ese ritmo durante 10 minutos.", "Respiración controlada durante todo el esfuerzo."], tip: "El 85% es zona de umbral: alta intensidad sostenible." },
  ...
  "Recuperación activa": { muscles: "Recuperación cardiovascular", steps: ["Caminá de regreso al punto de inicio.", "Inhalá por la nariz, exhalá por la boca.", "Mantené el movimiento — no pares completamente.", "40 segundos entre cada sprint."], tip: "La calidad de recuperación determina la calidad del siguiente sprint." },
  ...
  "Intervalos en cinta o bicicleta": { muscles: "Cardiovascular · Tren inferior · Resistencia aeróbica", steps: ["Calentá 2 minutos a ritmo suave.", "Subí al 75% de esfuerzo máximo durante 90 segundos.", "Bajá a caminata activa durante 60 segundos.", "Repetí 4 rondas completas.", "Enfriá 2 minutos al final."], tip: "El 75% es cuando podés hablar pero te cuesta." },
  "Dead bug": { muscles: "Core (transverso · oblicuos) · Estabilización lumbar", steps: ["Acostada boca arriba, brazos hacia el techo, rodillas a 90° en el aire.", "Pegá la zona lumbar completamente al piso.", "Extendé el brazo derecho hacia atrás y la pierna izquierda simultáneamente.", "Volvé al centro y repetí del otro lado.", "Movimiento lento y controlado."], tip: "Si la zona lumbar se despega del piso, reducí el rango de movimiento." },
...
const ROUTINES = [
  {
    ...
    days: [
      { id:1, label:"DÍA 1", title:"Potencia & Tren Inferior", accent:"#3B82F6", accentDark:"#1D4ED8", emoji:"🦵", phases:[
        ...
        { id:"str", label:"FUERZA", duration:30, color:"#3B82F6", exercises:[{name:"Sentadilla Frontal",detail:"4 × 5 — carga media-alta",timer:300},{name:"Romanian Deadlift",detail:"3 × 8 — excéntrico 3s",timer:240},{name:"Bulgarian Split Squat",detail:"3 × 8/lado",timer:240},{name:"Pallof Press",detail:"3 × 10/lado — antirotación",timer:180}]},
      ...
      { id:2, label:"DÍA 2", title:"Tren Superior & Hombros", accent:"#10B981", accentDark:"#047857", emoji:"💪", phases:[
        { id:"mob", label:"MOVILIDAD", duration:8, color:"#06B6D4", exercises:[{name:"Rotación Torácica cuadrupedia",detail:"10 reps / lado",timer:60},{name:"Wall Slide",detail:"10 reps — activa manguito",timer:60},{name:"Child's Pose apertura lateral",detail:"30\" por lado",timer:60}]},
        { id:"str", label:"FUERZA", duration:30, color:"#10B981", exercises:[{name:"Dominadas con lastre",detail:"4 × 5-6",timer:300},{name:"Press banca mancuernas",detail:"3 × 8 — codos a 45°",timer:240},{name:"Face Pull con cuerda",detail:"3 × 15 — OBLIGATORIO",timer:180},{name:"Remo Yates / Pendlay Row",detail:"3 × 6",timer:240},{name:"Curl + Press Arnold",detail:"2 × 10",timer:180}]},
      ...
      { id:3, label:"DÍA 3", title:"Atletismo & Core", accent:"#8B5CF6", accentDark:"#6D28D9", emoji:"⚡", phases:[
        ...
        { id:"str", label:"FUERZA", duration:30, color:"#8B5CF6", exercises:[{name:"Peso Muerto Convencional",detail:"4 × 4 — carga alta",timer:360},{name:"Swing con Kettlebell",detail:"4 × 12 — potencia cadera",timer:240},{name:"Step-up con mancuernas",detail:"3 × 10/lado",timer:240},{name:"Plank con arrastre",detail:"3 × 8/lado",timer:180},{name:"Turkish Get-Up",detail:"2 × 3/lado",timer:300}]},
        { id:"fin", label:"FINALIZADOR", duration:10, color:"#F59E0B", exercises:[{name:"Sprint 20m",detail:"5 sprints — máx aceleración",timer:40},{name:"Recuperación activa",detail:"40\" caminando",timer:40}]},
      ]},
    ],
  },
  {
    ...
    days: [
      { id:1, label:"DÍA 1", title:"Glúteos + Piernas + Core", accent:"#2B5C3F", accentDark:"#1A3D2A", emoji:"🦵", phases:[
        ...
        { id:"str", label:"FUERZA", duration:30, color:"#2B5C3F", exercises:[{name:"Goblet squat",detail:"4 × 10 — profundidad completa",timer:180},{name:"Romanian Deadlift",detail:"4 × 8 — bisagra de cadera",timer:180},{name:"Hip thrust",detail:"3 × 12 — glúteos apretados",timer:150},{name:"Pallof Press",detail:"3 × 10/lado — core firme",timer:120}]},
      ...
      { id:2, label:"DÍA 2", title:"Abdomen + Tren superior", accent:"#3D3080", accentDark:"#28205A", emoji:"💪", phases:[
        ...
        { id:"str", label:"FUERZA", duration:26, color:"#3D3080", exercises:[{name:"Dead bug",detail:"3 × 8 reps lentas / lado",timer:120},{name:"Remo con mancuerna",detail:"3 × 10/lado",timer:150},{name:"Face Pull con cuerda",detail:"3 × 15 — codos arriba",timer:90},{name:"Sumo squat con mancuerna",detail:"3 × 12 — pies 45°",timer:120},{name:"Farmer carry",detail:"3 × 20 metros",timer:60}]},
      ...
      { id:3, label:"DÍA 3", title:"Potencia + Piernas unilaterales", accent:"#7A3A10", accentDark:"#4A2008", emoji:"⚡", phases:[
        ...
        { id:"str", label:"POTENCIA Y FUERZA", duration:30, color:"#7A3A10", exercises:[{name:"Sentadilla con salto",detail:"4 × 5 — salida explosiva",timer:120},{name:"Bulgarian Split Squat",detail:"3 × 8/lado",timer:180},{name:"Landmine rotation",detail:"3 × 10/lado",timer:150},{name:"Pull-up o jalón al pecho",detail:"3 × 8",timer:150},{name:"Abducción de cadera en máquina",detail:"3 × 15 — bajada lenta",timer:90}]},
...
function ExRow({ ex, color, num, checked, onCheck, onInfo }: any) {
  ...
  return(
    <div style={{background:checked?"#052e16":"#0F172A",border:`1px solid ${checked?"#166534":"#1E293B"}`,borderRadius:12,marginBottom:8,overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px"}}>
        ...
        <div style={{flex:1,minWidth:0}} onClick={()=>ex.timer>0&&setOpen((o:boolean)=>!o)}>
        ...
        {ex.timer>0&&<div onClick={()=>setOpen((o:boolean)=>!o)} style={{color:"#475569",fontSize:13,cursor:"pointer",transform:open?"rotate(180deg)":"none",flexShrink:0}}>▼</div>}
      </div>
      {open&&ex.timer>0&&(
        <div style={{padding:"0 14px 14px",borderTop:"1px solid #1E293B"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginTop:10,flexWrap:"wrap"}}>
            <div style={{width:100,height:5,background:"#1E293B",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:done?"#22C55E":color,borderRadius:3,transition:"width 1s linear"}}/></div>
            <span style={{fontSize:14,fontWeight:700,color:done?"#22C55E":"#F1F5F9",minWidth:42}}>{done?"✓ listo":fmt(secs)}</span>
            {!done&&<button onClick={()=>setRunning((r:boolean)=>!r)} style={{background:running?"#334155":color,border:"none",borderRadius:8,padding:"5px 14px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>{running?"⏸ Pausa":"▶ Start"}</button>}
...
function SessionClock({ accent }: { accent: string }) {
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:12,background:"#0F172A",borderRadius:14,padding:"12px 16px",border:`1px solid ${over?"#EF4444":"#1E293B"}`,marginBottom:20}}>
      ...
      <button onClick={()=>setRunning((r:boolean)=>!r)} style={{background:running?"#1E293B":accent,border:"none",borderRadius:10,padding:"8px 16px",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>{running?"⏸ Pausa":elapsed===0?"▶ Iniciar":"▶ Seguir"}</button>
      {elapsed>0&&!running&&<button onClick={()=>{setElapsed(0);setRunning(false)}} style={{background:"transparent",border:"1px solid #334155",borderRadius:10,padding:"8px 10px",color:"#64748B",fontSize:13,cursor:"pointer"}}>↺</button>}
    </div>
  )
}
    ...
    <div style={bg}>
      <div style={{maxWidth:440,margin:'0 auto',padding:'24px 16px'}}>
        ...
        {routine!.days.map((d:any,i:number)=>(
          <div key={d.id} onClick={()=>{setDayIdx(i);setChecks({});setLog([])}} style={{background:'#0F172A',border:'1px solid #1E293B',borderRadius:18,padding:'18px 20px',marginBottom:12,cursor:'pointer',display:'flex',alignItems:'center',gap:16}}>
  ...
  return (
    <div style={bg}>
      ...
      <div style={{maxWidth:480,margin:'0 auto',padding:'20px 16px 48px'}}>
        ...
        <SessionClock accent={day!.accent}/>
          ...
          return(
            <div key={phase.id} style={{marginBottom:24}}>
              ...
              {phase.exercises.map((ex:any,ei:number)=>(
                <ExRow key={ei} ex={ex} color={phase.color} num={ei+1} checked={!!checks[`${pi}-${ei}`]} onCheck={()=>toggleCheck(`${pi}-${ei}`,ex.name)} onInfo={()=>setModal(ex.name)}/>
              ))}
            </div>
          )
        })}
        {allDone&&(
          <div style={{background:`linear-gradient(135deg,${day!.accentDark},${day!.accent})`,borderRadius:16,padding:'22px',textAlign:'center',marginTop:8}}>
            <div style={{fontSize:40,marginBottom:8}}>🎉</div>
            <div style={{fontSize:18,fontWeight:800,color:'#fff',marginBottom:4}}>¡Sesión completada!</div>
            <div style={{fontSize:13,color:'rgba(255,255,255,0.8)'}}>{day!.label} — {day!.title}</div>
          </div>
        )}
        {log.length>0&&(
